...
MONITOR_DICT = {"ingestion": ["dump"], "annotation" : ["ingestion"], "backend": ["annotation", "mlops"], "mlops": ["backend"], "monitor": ["dump", "ingestion", "annotation", "backend", "mlops"]}

# allowed user input strings
dump_opts = ["dump", "storage", "offload", "drop"]
ingest_opts = ["ingestion", "ingest", "ingestor", "processor"]
annotation_opts = ["annotation", "annotator", "labels", "labeler", "labeling", "annotations"]
backend_opts = ["backend", "curation", "data curation", "dataset", "dataset builder"]
mlops_opts = ["mlops", "train", "training", "eval", "evaluation", "testing", "test", "retrain", "retraining", "fine-tune", "fine-tuning", "fine tune", "fine tuning", "transfer learning", "etl", "preprocessing", "augmentation", "augments"]
monitor_opts = ["monitor", "monitoring", "db monitor", "pipeline monitor"]

ALLOWED_PRODUCERS_CONSUMERS = {"dump" : dump_opts, "ingestion": ingest_opts, "annotation": annotation_opts, "backend": backend_opts, "mlops": mlops_opts, "monitor": monitor_opts}
...
def set_producer_consumer_type(user_input: str) -> str:
    ...
    for pc_type, options in ALLOWED_PRODUCERS_CONSUMERS.items():
        if user_input.lower() in [opt for opt in options]:
            return pc_type
    logging.error(f"Failure to pass an appropriate type for the producer or consumer. Type {user_input} is not recognized")
    return None
